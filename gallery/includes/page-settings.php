            ...
            foreach ($results as $result) { ?>
                <div id="notification-<?php echo $result->ID; ?>">
                    ...
                    if ($action == 'loved')
                        ...
                        $display .= '' . get_avatar($result->userID, 16) . ' <i class="fas fa-fw fa-folder"></i> <a href="' . get_author_posts_url($result->userID) . '">' . $nickname . '</a> added a poster <a href="' . get_permalink($result->postID) . '">' . get_the_title($result->postID) . '</a> to a <a href="' . home_url('/') . get_the_ip_slug($ip_collections_page_id) . '/' . $result->postKeyID . '">collection</a> <time>' . $time . '</time>';
