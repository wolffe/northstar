...
function imagepress_notifications($atts, $content = null) {
    ...
    foreach($res as $line) {
        ...
        if($action == 'collected' && $user_ID == $authorID)
            $display .= '<div class="notification-item n' . $line->ID . ' ' . $class . '" data-id="' . $line->ID . '"><div class="navatar">' . get_avatar($line->userID, 48) . '</div><i class="fas fa-fw fa-folder"></i> <a href="' . get_author_posts_url($line->userID) . '">' . $nickname . '</a> added your poster <a href="' . get_permalink($line->postID) . '">' . get_the_title($line->postID) . '</a> to a collection <time>' . $time . '</time></div>';
